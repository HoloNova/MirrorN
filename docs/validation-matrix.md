...
`pnpm test:e2e` 共 18 条，覆盖 PLAN 6.2 要求的内容：

| 要求             | 用例位置                                                                        |
| ---------------- | ------------------------------------------------------------------------------- |
| 搜索             | `e2e/search.spec.ts`（键盘搜索、生态入口、清空、输入法组词、窄屏、关闭动画）    |
| 向导             | `e2e/search.spec.ts`、`e2e/ecosystems.spec.ts`（apt 版本切换、Docker 两项配置） |
| 手动选源不被覆盖 | `e2e/probe.spec.ts`「用户手动选过来源之后，重新测量不会替换选择和命令」         |
| API 离线降级     | `e2e/status.spec.ts`「后端不可用时按未知展示，向导本身仍然完整可用」            |
| 探测失败         | `e2e/probe.spec.ts`「超时的来源显示超时，并且不会被推荐」                       |

所有测速与后端请求都在测试里用 `page.route` 拦截，因此**不依赖上游站点的实时可用性**。
